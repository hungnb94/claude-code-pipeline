...
- `hooks/trigger_pipeline.js` (`UserPromptSubmit`) is unchanged — it already puts full step content on stdout, which Claude Code adds to context directly; it has no separate human-visibility gap to fix. If a future need arises to also give it a `systemMessage`-style notification, treat that as a new decision, not an extension of this one.
